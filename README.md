...
- C# 기본
    ...
    public enum FontStyle
    {
        ...
        Bold = 1,       // 굵은 글씨. 0001
        Italic = 2,     // 이탤릭 체. 0010
        Underline = 4,  // 글자 밑줄. 0100
        Strikeout = 8,  // 취소선 .   1000
    }
    ``` 
    - 모달창, 모달리스창 : 창위에 다른 창을 띄울 때 접근권한 제어 형태
        - Modal : 부모창과 연관된 작업을 할 
        때 사용. 모달창이 닫히지 않으면 부모창을 제어할 수 없음
        - Modaless: 부모창과 상관없이 동작, 사용 주의. 메인창을 언제나 닫을 수 있고, 메인 창을 닫으면 모두 종료. 사용 시 주의
        - 메시지 박스 : 기본적으로 모달창으로 동작
- C# 고급
...
- 데이터베이스 데이터 바인딩
    ...
    <img src="./image/cs0016.png" width="600">s
...
- 사용법 - [소스](./day08/Day08Study/WpfStudyApp05/MainWindow.xaml)
    ...
    4. App.xaml에 필요한 리소스 코드 복사/붙여넣기
...
- 기본 사용법 - [소스](./day09/Day09Study/WpfStudyApp03/MainWindow.xaml)
    ...
    - App.xaml.cs 오픈 - [소스](./day09/Day09Study/WpfStudyApp03/App.xaml.cs)
        ...
        private async void OnStartup(object sender, StartupEventArgs e)
        {
            ...
            // .. MainWindow 인스턴스 생성
            var mainWindow = _host.Services.Get
        }
        ```
...
- 주요 기능
<img src="./image/cs0026.png" width="400">

- 펜
    - 펜종류 : 펜, 브러쉬, 마커, 크레용, 스프레이 등 다양한 브러쉬 구현
    - 펜 사이즈 조절 기능 (브러쉬는 추후 수정 예정)
    - 펜 색상: 클릭시 colorDialog 창과 함께 펜의 색상을 선택할 수 있음, 도형에도 적용됨
- 지우개
    - 그림을 그리기 시작하면 지우개 버튼 활성화
    - 배경지우개: 배경만 지울 수 있는 지우개 (그림 안지워짐)
    - 그림 초기화: 사용자가 그린 그림만 초기화
    - 배경 초기화: 사용자가 추가한 배경만 초기화됨 (흰 바탕으로)
    - 전체 초기화 : 그림, 배경 모두 초기화, redo, undo 도 초기화

- 도형
    - 채우기 도형을 클릭하면 색이 채워진 도형을 그릴 수 있음
- 파일
    - 이미지 불러오기: 사용자 컴퓨터의 이미지를 가져와 배경으로 칠함
    - 배경유지 저장: 배경을 유지한 채 저장
    - 배경없이 저장: 투명 배경으로 저장(확장자 .png일 경우)

- 기타
    - 기본 캔버스 색을 변경할 수 있음  (배경으로 취급)

- undo/redo
    - 사용자가 그린 그림을 되돌리고, 되돌린 것을 취소할 수 있음
...
- 추후 개선 사항
    - 브러쉬로 드로잉할 경우 끊어짐 현상 보완
    - 레이어 기능
    - 그림판을 닫을 경우 저장할건지 묻는 메시지 창 추가
